...
%client send a request to the web container in the form of http request
%the request is handeled by the Front controller (DispatcherServlet) which findes the appropriate Handler Mappings 
%The Handelr MAppiing helps the DisåatcherServlet send the request to the right Controller. 
%The controler processes the request and returns a view in the for of a ModelAndView instance that's sent to the Front Controller. 
%The Front Controller resolves the view by consulting the View Resolver
%The view is then returned to the client. 
